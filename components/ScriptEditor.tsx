...
export const ScriptEditor: React.FC<ScriptEditorProps> = ({ 
  ...
  const playBuffer = async (buffer: AudioBuffer, key: string) => {
    ...
    setPlayingKey(key);
    source.onended = () => { if (playingKey === key) setPlayingKey(null); };
    source.start();
  };
  ...
  const handleAction = async (lineText: string, action: 'play' | 'download' | 'reset') => {
    ...
    if (action === 'play') {
      ...
      if (audioCache[key]) { await playBuffer(audioCache[key], key); return; }
  ...
  const renderHighlightedLine = (line: string) => {
    ...
    return parts.map((part, i) => {
      ...
      if (part.startsWith('(') && part.endsWith(')')) return <span key={i} className="text-pink-400 font-medium">{part}</span>;
          ...
          {lines.map((line, i) => {
            ...
            return (
              <div key={i} className="h-7 mb-1 flex items-center justify-center gap-1 group/btn px-1">
                {isDialogue && (
                  <>
                    <button onClick={() => handleAction(line, 'play')} className={`p-1 rounded hover:bg-slate-700 transition-colors ${isPlaying ? 'text-green-400' : 'text-pink-400'}`}>
